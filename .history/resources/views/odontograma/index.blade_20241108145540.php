...
<main>
  ...
  {{--  @if(isset($paciente))  --}}
      ...
      <form action="{{ route('odontograma.registrar') }}" method="POST">
        ...
        <input type="number" name="num_diente" id="num_diente" required>
...
<script>
    // Cargar el carnet del paciente en el campo oculto al cargar la página
    document.addEventListener('DOMContentLoaded', () => {
        const savedCarnet = localStorage.getItem('carnet');
        if (savedCarnet) {
            document.getElementById('carnet_hidden').value = savedCarnet;
        }
    });
</script>
