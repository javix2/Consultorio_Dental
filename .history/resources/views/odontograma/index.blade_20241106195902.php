                    ...
                    <tr>
                        ...
                        <td>
                            <form action="{{ route('pago.destroy', $odontograma->id) }}" method="POST" style="display: inline-block;">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('¿Estás seguro de eliminar este registro?')"><i class="fa-light fa-trash-can">Eliminar</i></button>
                            </form>            
                        </td>
                    </tr>
                @endforeach
