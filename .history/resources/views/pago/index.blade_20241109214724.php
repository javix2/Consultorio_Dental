            ...
            @foreach($histo_tratams as $histo)
                <tr>
                    ...
                    <td>
                        ...
                        <a href="" class="btn btn-success btn-sm" data-bs-toggle="modal" data-bs-target="#modalPagar{{$histo->id}}"><i class="fa-solid fa-money-bill-wave"></i></i></a>
                        
                        <form action="{{ route('pago.destroy', $histo->id) }}" method="POST" style="display: inline-block;">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('¿Estás seguro de eliminar este registro?')"><i class="fa-regular fa-trash-can"></i></button>
                        </form>
                        {{--  @include('pago.editar')
                        @include('pago.ver')  --}}
                        @include('pago.pagar')
