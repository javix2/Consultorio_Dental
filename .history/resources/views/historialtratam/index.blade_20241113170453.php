  ...
  <table class="table table-striped table-hover ">
    ...
    <tbody class="table-group-divider">
      ...
      <tr>
        ...
        <td>
          @if ($histo->sesions->first())
              <span class="badge 
                  @if ($histo->sesions->first()->estado === 'en curso') bg-warning
                  @elseif ($histo->sesions->first()->estado === 'terminado') bg-success
                  @elseif ($histo->sesions->first()->estado === 'pendiente') bg-danger
                  @else bg-secondary
                  @endif">
                  {{ $histo->sesions->first()->estado }}
              </span>
          @else
              <span class="badge bg-secondary">No hay sesiones</span>
          @endif
      </td>
      

        {{--  <td>
          @if($histo->estado==="en curso")
        ...
        <td>
            ...
            <a href="" class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#modalSesion{{$histo->id}}"><i class="fa-regular fa-circle-check"></i></a>
            
            <form action="{{ route('historialtratam.elimina',$histo->id) }}" method="POST" style="display: inline-block;">
                @csrf
                @method('DELETE')
                <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('¿Estás seguro de eliminar este registro?')"><i class="fa-solid fa-trash"></i></button>
            </form>
