...
class OdontogramaController extends Controller
{
    ...
    public function registrar(Request $request) 
    {
        ...
        $validatedData = $request->validate([
            'num_diente' => 'required',
            'lado_diente' => 'required',
            'estado' => 'required',
            'carnet' => 'required|exists:pacientes,carnet', // Asegurarse de que el carnet exista en la tabla de pacientes
        ]);

        // Buscar el paciente por el carnet
        $paciente = Paciente::where('carnet', $validatedData['carnet'])->first();

        if (!$paciente) {
            return redirect()->back()->with('error', 'Paciente no encontrado.');
        }

        // Crear el nuevo registro del odontograma
        // Odontograma::create([
        //     'num_diente' => $request->num_diente,
        //     'lado_diente' => $request->lado_diente,
        //     'estado' => $request->estado,
        //     'paciente_id' => $request->carnet,
        // ]);
         // Crear el nuevo odontograma y asociarlo con el paciente
        ...
        $odontograma->paciente_id = $paciente->id; // Asociar el odontograma con el ID del paciente
        $odontograma->save();


        return redirect()->back()->with('success', 'creado con éxito.');

        // Respuesta en formato JSON con los datos del nuevo odontograma
        // return response()->json([
        //     'id' => $odontograma->id,
        //     'num_diente' => $odontograma->num_diente,
        //     'lado_diente' => $odontograma->lado_diente,
