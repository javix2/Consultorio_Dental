...
public function update(Request $request, $id)
{
    ...
    // Validar los campos, incluyendo la verificación de la contraseña actual solo si es necesario
    $request->validate([
        'current_password' => [
            'required',
            function ($attribute, $value, $fail) use ($user) {
        ...
        'email' => 'required|email|max:255|unique:users,email,' . $user->id,
        'password' => [
            'nullable',      // La nueva contraseña es opcional
            'min:8',         // Mínimo 8 caracteres si se proporciona
            'confirmed',     // Debe coincidir con password_confirmation
            function ($attribute, $value, $fail) use ($user) {
                if ($value && Hash::check($value, $user->password)) {
                    $fail('La nueva contraseña debe ser diferente a la contraseña actual.');
                }
            },
        ],
    ]);

    // Actualizar solo el nombre y el correo electrónico del usuario
    $user->name = $request->name;
    $user->email = $request->email;
